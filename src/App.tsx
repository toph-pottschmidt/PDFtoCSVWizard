...
const VERSION = "0.2.0" // TODO: pull from github
...
type CellContentObject = Operation | TemplateObject

type CellContent = CellContentObject[]

function App() {
    // local storage values and callbacks
    // TODO: figure out "correct" usage of this without incurring unused variables
    ...
    // TODO: this is 1-index, figure out if this causes issues
    ...
    const editData = ({ rowIndex, colId, newValue }) => {
        ...
        if (updateAll && editingMode === MANUAL_MODE) {
            ...
            if (row) {
                ...
                console.warn(`no row found for ${rowIndex}, ${colId}`)
            }
        }
    }
    ...
    const onTextObjectClick = async (wordObj) => {
        ...
        const newWordObject: TemplateObject = {
            ...wordObj,
            page: currentPage,
        }

        const newValue = [...currentCellValue, newWordObject]
    ...
    const addNewRow = () => {
        ...
        api.applyTransaction({ add: [{ ...defaultRowValue, id: rowCount }] })
    }
    ...
    const clearCellData = () => {
        editData({ ...activeCell, newValue: [{ id: activeCell.rowIndex }] })
    }
    ...
    const adjustRowTemplateIndex = (rowIndex, value) => {
        ...
        const currentTemplate = api.getDisplayedRowAtIndex(rowIndex)?.data
        ...
        api.getDisplayedRowAtIndex(rowIndex)?.setData(newTemplateValue)
    }
    ...
    const deleteRow = useCallback((row: number) => {
        ...
        const id = api.getDisplayedRowAtIndex(row)?.id
                        ...
                        <Group wrap="nowrap">
                            ...
                            {Object.values(OPERATIONS).map((o) => (
                                <Button
                                    key={o.symbol}
                                    onClick={() =>
                                        appendOperationToCurrentCell(o)
                                    }
                            ...
                            <NumberInput
                                value={
                                    getFirstTemplateValue(
                                        ...
                                        )?.getDisplayedRowAtIndex(templateRow)
                                    )?.index
                                }
                                disabled={
                                    getFirstTemplateValue(
